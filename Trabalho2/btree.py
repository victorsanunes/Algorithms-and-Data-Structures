...
# Binary search of element x in array a
def find_it(a, x):
    lo, hi = 0, len(a)
    while hi != lo:
        m = (hi+lo)//2
...
# Class to raise when attempting to add a duplicated value
# does nothing
...
def displayText(index):
    path = "sourcetexts/text"+str(index+1)+".txt"
    file = open(path, "r")
    text = file.readlines()
    file.close()
    return text
...
class BTree(BaseSet):
    class Node(object):
        ...
        # ci: id do filho do no
        def add(self, x, ci):
            i = find_it(self.keys, x)
            if i < 0: return False

            # ======== ABRE ESPACO PARA INSERIR COPIAR AS CHAVES ==============
    ...
    # ========= ADICIONA O NO X NA FOLHA E VOLTA PARA A RAIZ ===================
    # ============== FAZENDO SLIP DOS NOS QUE JA ESTAO CHEIOS ==================
    def add(self, x):
        w = None
        try:
    ...
    # Adiciona o valor x na subarvore com identificador ui
    def add_recursive(self, x, ui):
        ...
        # Checa se u eh uma folha
        if u.children[i] < 0:
            # Adiciona o noh no final
            u.add(x, -1)
    ...
    def find(self, x):
        ...
        ui = self.ri
        while ui >= 0:
            # u is a node that simulates a blok in the hard disk
            u = self.bs.read_block(ui)

            # i is the position of the element
            i = find_it(u.keys, x)

            # if i is < 0, then the elemente wasnt found yet.
            # Returns the smallest integer such taht u.children > x
            if i < 0:
