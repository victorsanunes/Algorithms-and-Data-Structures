import bisect     # Biblioteca para inserir ordenadamente em uma lista
import itertools
import operator
import random
# import unittest
...
class _BNode(object):
    ...
    #Construtor do no
    def __init__(self, tree, contents=None, children=None, text=None, counting=None):
        self.tree = tree
        self.contents = contents or []
        self.children = children or []

        #Checa se o no estourou ou nao o limite de dados
        if self.children:
            assert len(self.contents) + 1 == len(self.children), \
                    "one more child than data item required"

    # Reimplementacao da funcao de impressao
    def __repr__(self):
...
class BTree(object):
    ...
    def _path_to_text(self, item):
        """

        """
        current = self._root
        ancestry = []
        # if item[0] in list(current):
        # Caso tenha filhos
        i = j = 0
        while getattr(current,"children", None):
            for c1 in current.children:
        ...
        #     # Retorna a posicao de insercao da parte mais a esquerda
        #     # index = bisect.bisect_left(current.contents, item)
        #     index = find_it(current.contents, item)
        #     # Adiciona essa informacao ao pai
        #     ancestry.append((current, index))
        #
        #     # Checa se o item ja esta inserido
    ...
    def _present(self, item, ancestors):
        # Pega o ulitmo current e index inseridos no ancestral
        last, index = ancestors[-1]
        return index < len(last.contents) and last.contents[index] == item

    def _textPresent(self, item, ancestors):
        # Pega o ulitmo current e index inseridos no ancestral
        last, index = ancestors[-1]
        for i in last.contents:
            print("i: " + str(i))
            if item in i:
                return True
        print("contents: " + str(last.contents))
        return index < len(last.contents) and item in last.contents[index]

    def search(self, item):
        current = self._root
        ancestors = self._path_to_text(item)

        print("ancestors: " + str(ancestors))
        # if self._textPresent(item, ancestors):
        #     node, index = ancestors.pop()
        #
        #     print("TESTE: " + str(item[1]))
        # else:
    ...
    def insert(self, item):
        ...
        # Se o ulitmo for crianca
        while getattr(node, "children", None):
            # Busca o no filho
            node = node.children[index]

            # Acha a posicao de insercao mais a esquerda
            index = bisect.bisect_left(node.contents, item)

            # Adiciona essas informacoes ao ancestral
            ancestors.append((node, index, word))
...
def main():
    bNode = BTree(2)
    text, stopwords = pre.preProcessText(text_path="sourcetexts/text01.txt",
                                        stopwords_path="sourcetexts/pt_stopwords.txt")

    final_text = [x for x in text if x not in stopwords]
    # bNode.insert([1, final_text])
    bNode.insert(["c", 10])
    bNode.insert(["h", 20])
    bNode.insert(["f", 30])
    bNode.insert(["g", 40])
